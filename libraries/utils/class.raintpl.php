...
class RainTPL{
	...
	function draw( $tpl_name, $return_string = false ){
       ...
       if( !$this->cache && !$return_string ){
            ...
            if( $this->cache )
                file_put_contents( $this->tpl['cache_filename'], "<?php defined('ROOT') or exit(); ?".">" . $raintpl_contents );
            unset( $this->tpl );
	...
	protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		//compile template
		$template_compiled = "<?php defined('ROOT') or exit(); ?".">" . $this->compileTemplate( $template_code, $tpl_basedir );
